...
async function importSessionBundle(database, rawSessionId, session = '0') {
  ...
  if (await credentials.count({ where: { session } })) return false
  ...
  return database.transaction(async transaction => {
    ...
    if (rows.length) await keys.bulkCreate(rows, { ignoreDuplicates: true, transaction })
    return true
  })
}
