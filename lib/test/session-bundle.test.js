...
test('session bundle round trips credentials and key files', () => {
  ...
  assert.match(sessionName(id), /^levanter_[a-f0-9]{40}$/)
})

test('decodes legacy gzip session bundles', () => {
  const files = { 'creds.json': { registered: true } }
  const legacy = 'KENTECH_' + gzipSync(serialize({ version: 2, files })).toString('base64url')
  assert.deepEqual(decodeSession(legacy), files)
})
...
test('imports credentials and converts multi-file key names to database keys', async () => {
  ...
  const database = { sync: async () => {}, transaction: async callback => callback({}), models: {
  ...
  assert.equal(created[0].session, '0')
