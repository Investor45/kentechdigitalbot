...
function setup() {
  ...
  const message = {
    ...
    client: {
      groupMetadata: async () => ({ participants: [{ id: '111@s.whatsapp.net' }, { id: '222@s.whatsapp.net' }] }),
      sendMessage: async (jid, payload, options) => {
        statusMessages.push({ jid, payload, options })
        return { key: { id: 'status-1' } }
      },
    },
    setStatus: async (_, jids) => jids.forEach(jid => posted.push(jid)),
    groupStatus: async (_, jid) => { posted.push(jid); await new Promise(setImmediate) },
    send: async text => sent.push(text),
  }
  return { entries, message, posted, sent, statusMessages,
...
test('sends replied media through the status broadcast payload', async () => {
  const { command, message, posted, statusMessages } = setup()
  message.reply_message.downloadMediaMessage = async () => Buffer.from('image')
  await command.handler(message, '')
  assert.equal(statusMessages.length, 1)
  assert.equal(statusMessages[0].jid, 'status@broadcast')
  assert.ok(statusMessages[0].payload.image)
  assert.deepEqual(Array.from(statusMessages[0].options.statusJidList), ['111@s.whatsapp.net', '222@s.whatsapp.net'])
  assert.deepEqual(posted, [])
})

test('sends replied text through the status broadcast payload', async () => {
  const { command, message, statusMessages } = setup()
  message.reply_message = { txt: true, text: 'hello' }
  await command.handler(message, '')
  assert.equal(statusMessages[0].payload.text, 'hello')
})

test('group status cannot be posted by a non-owner', async () => {
  const { listener, message, posted, sent } = setup()
  message.fromMe = true
