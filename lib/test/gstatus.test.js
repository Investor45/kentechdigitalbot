...
test('uses the native status pipeline when available for media payloads', async () => {
  const { command, message, posted } = setup()
  const calls = []
  message.setStatus = async (...args) => calls.push(args)
  await command.handler(message, '')
  assert.equal(calls.length, 1)
  assert.deepEqual(Array.from(calls[0][1]), ['123@g.us'])
  assert.equal(calls[0][2], '123@g.us')
  assert.deepEqual(posted, [])
})

test('falls back when the native status audience is rejected', async () => {
  const { command, message, posted } = setup()
  message.setStatus = async () => { throw new Error('audience rejected') }
  await command.handler(message, '')
  assert.deepEqual(posted, ['123@g.us'])
})
