...
function registerOwnerCommand(bot, name, handler, desc, aliases = []) {
  ...
  const run = async (message, match) => {
    // Group status is a normal user command. It must remain usable even when
    // the sender is not the configured owner; the destination group and
    // WhatsApp permissions still decide whether the post can be delivered.
    if (!isOwner(message) && name !== 'gstatus') return
    const key = messageKey(message)
